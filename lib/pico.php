...
class Pico {
  ...
  protected function limit_words($string, $word_limit)
  {
    ...
    $string = preg_replace('#/\*.+?\*/#s', '', $string); // Remove comments and meta
    $string = str_replace('%base_url%', $this->base_url(), $string);

    // limits excerpt only to first paragraph
    /* 
    $string = ltrim($string) . "\n";
    ...
    */
    ...
    $content = MarkdownExtra::defaultTransform($excerpt);
    //$this->parse_content($page_content)
